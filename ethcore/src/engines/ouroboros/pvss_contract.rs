...
mod provider {
    ...
    impl Contract {
    	pub fn new<F>(address: util::Address, do_call: F) -> Self
    		...
    		Contract {
    			contract: ethabi::Contract::new(ethabi::Interface::load(b"[{\"constant\":false,\"inputs\":[{\"name\":\"epochIndex\",\"type\":\"uint64\"},{\"name\":\"sender\",\"type\":\"address\"}],\"name\":\"getCommitment\",\"outputs\":[{\"name\":\"\",\"type\":\"bytes\"}],\"payable\":false,\"type\":\"function\"},{\"constant\":false,\"inputs\":[{\"name\":\"epochIndex\",\"type\":\"uint64\"},{\"name\":\"commitment\",\"type\":\"bytes\"}],\"name\":\"saveCommitment\",\"outputs\":[],\"payable\":false,\"type\":\"function\"}]").expect("JSON is autogenerated; qed")),
    			address: address,
    	...
    	/// Auto-generated from: `{"constant":false,"inputs":[{"name":"epochIndex","type":"uint64"},{"name":"sender","type":"address"}],"name":"getCommitment","outputs":[{"name":"","type":"bytes"}],"payable":false,"type":"function"}`
    	#[allow(dead_code)]
    	pub fn get_commitment(&self, epoch_index: u64, sender: &util::Address) -> Result<Vec<u8>, String>
    		 {
    	...
    	/// Auto-generated from: `{"constant":false,"inputs":[{"name":"epochIndex","type":"uint64"},{"name":"commitment","type":"bytes"}],"name":"saveCommitment","outputs":[],"payable":false,"type":"function"}`
    	#[allow(dead_code)]
    	pub fn save_commitment(&self, epoch_index: u64, commitment: &[u8]) -> Result<(), String>
    		 {
    		let call = self.contract.function("saveCommitment".into()).map_err(Self::as_string)?;
    		let data = call.encode_call(
    			vec![ethabi::Token::Uint({ let mut r = [0u8; 32]; util::U256::from(epoch_index as u64).to_big_endian(&mut r); r }), ethabi::Token::Bytes(commitment.to_owned())]
    		).map_err(Self::as_string)?;
